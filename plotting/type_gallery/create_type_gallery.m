...
types_to_plot = cell_types(40:end);
types_to_plot = {'82wo'};
...
for t = 1:length(types_to_plot)
    ...
    text(0.02,0.5,types_to_plot{t},'Units','normalized','FontName','Arial','FontSize',40);
    axis off;
    ...
    text(0.5,0.5,'a','Units','normalized','FontName','Arial','FontSize',25);
    axis off;
    ...
    line([0 5376],[744 744],'LineStyle','--','LineWidth',2,'Color',[192 192 192]/255);
    line([0 5376],[919 919],'LineStyle','--','LineWidth',2,'Color',[192 192 192]/255);
    
    labelb_axes = axes('Position',[0.03 0.28 0.02 0.02]);
    text(0.5,0.5,'b','Units','normalized','FontName','Arial','FontSize',25);
    axis off;
    ...
    strat_bin = strat_bin/bin_res;
    hold on;
    plot(x,strat_bin,'Color',[119 136 153]/255,'LineWidth',6);
    
    
    strat_ylim = get(gca,'YLim');
    line([0.28 0.28],strat_ylim,'LineStyle','--','Color',[192 192 192]/255);
    line([0.62 0.62],strat_ylim,'LineStyle','--','Color',[192 192 192]/255);
    
    xlim([-0.1 1.1]);
    xlabel('IPL Depth','FontName','Arial','FontSize',15);
    ylabel('Skeleton Density','FontName','Arial','FontSize',15);
    set(gca,'XTick',[0 0.28 0.45 0.62 1],'XTickLabel',{'0','Off SAC','0.45','On SAC','1'},'FontName','Arial','FontSize',13);
    
    labelc_axes = axes('Position',[0.45 0.95 0.02 0.02]);
    text(0.5,0.5,'c','Units','normalized','FontName','Arial','FontSize',25);
    axis off;
    ...
    text(0.5,0.5,'d','Units','normalized','FontName','Arial','FontSize',25);
    axis off;
    
    
    % Temporal response panel
    temp_axes = axes('Position',[0.75 0.05 0.2 0.4]);
    ...
    hold on;
    plot(x,temp_resp,'Color',[119 136 153]/255,'LineWidth',6);
    temp_ylim = get(gca,'YLim');
    line([x(8) x(8)],temp_ylim,'LineStyle','--','Color',[192 192 192]/255);
    line([x(16) x(16)],temp_ylim,'LineStyle','--','Color',[192 192 192]/255);
    
    xlim([0 4]);
    xlabel('Time (s)','FontName','Arial','FontSize',15);
    ylabel('\DeltaF/F (%)','Interpreter','tex','FontName','Arial','FontSize',15);
    set(gca,'FontName','Arial','FontSize',13);
    
    labele_axes = axes('Position',[0.70 0.45 0.02 0.02]);
    text(0.5,0.5,'e','Units','normalized','FontName','Arial','FontSize',25);
    axis off;
